...
- 论文摘要结构化分析
- 轻量 RAG 证据链：基于标题、摘要与结构化分析片段做章节级证据召回
- 文献分类与对比
- 综述大纲生成
- 综述正文生成
- 引用编号校验
...
- RAG 层使用轻量关键词召回，不依赖本地向量模型或向量数据库，适合普通课程服务器部署。
- 当前工作流是项目内自定义 Agent 串联，便于课程设计讲解和调试。

## 求职展示点

这个项目可以作为 AI 应用开发方向的实习作品集项目。建议重点讲清楚三件事：

- 完整 Agent 流程：主题理解、双源检索、排序、阅读、组织、写作、引用校验。
- 轻量 RAG 设计：基于真实论文构建证据片段库，按综述章节召回 evidence 后再生成正文。
- 工程化交付：FastAPI API、MySQL 持久化、Vue 工作台、任务状态轮询、Markdown 导出。
- 可靠性设计：外部 LLM 配置化、API 重试、结构化 JSON 解析、无结果任务失败处理、轻量排序策略。

更完整的项目讲解、面试表达和后续优化路线见 [项目讲解文档](docs/PROJECT_EXPLANATION.md)。
